...
function renderMaster(data, id) {
  ...
  for (let i=0; i<4; ++i) {
    labelsDiv.innerHTML += 
      `項目${i+1}: <input type="text" style="font-size: 1em; height: 22px;" id="label${i}" value="${data.labels[i]||defaultLabels[i]}"><br>`;
  }
  let html = "<h3>投票状況</h3>";
  for (let i=0; i<4; ++i) {
    html += `${data.labels[i]||defaultLabels[i]} : <span style="font-size: 2em; font-color: #f20; text-decoration: bold; font-family: Courier;">${data.votes[i]||0}</span>票<br>`;
  }
