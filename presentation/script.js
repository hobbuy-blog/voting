/**
 * Web Slides Creator - Core App Scripts
 * Fully optimized for touch devices and strictly layered event tracking
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 1. Ribbon Tab Router (Safe Event Setup for iPad Support) ---
    ...
    // Safe Event Binding Utility Helper
    const bindClick = (id, callback) => {
        const element = document.getElementById(id);
        if (element) element.onclick = callback;
    };

    // --- 2. History State Engine (Undo / Redo Architecture) ---
    function saveState() {
        if (!canvas) return;
        const currentSlide = slides[currentSlideIndex];
        const currentHTML = canvas.innerHTML;
        
        if (currentSlide.undoStack.length > 0 && currentSlide.undoStack[currentSlide.undoStack.length - 1] === currentHTML) return;
        
        currentSlide.undoStack.push(currentHTML);
        currentSlide.redoStack = []; // Flush redo stack on new operation
        updateUndoRedoUI();
    }
    ...
    // --- 3. Component Generation Engine ---
    function createBaseElement(typeClass, defaultWidth, defaultHeight) {
        saveState();
        ...
        el.style.touchAction = 'none'; // Lock default Safari scaling loops
    ...
    bindClick('add-text-btn', () => {
        ...
        inner.style.fontFamily = "'Inter', 'Noto Sans JP', sans-serif";
        inner.innerText = "Double tap to edit text";
        el.appendChild(inner);
        inner.onblur = () => saveState();
    });

    bindClick('add-rect-btn', () => {
        const el = createBaseElement('rect-element', 180, 180);
        const inner = document.createElement('div');
        inner.className = 'content-wrapper';
    ...
    if (imageUpload) {
        ...
        imageUpload.onchange = (e) => {
            ...
            reader.onload = (ev) => {
                const el = createBaseElement('image-element', 320, 240);
    ...
    // --- 4. High-Performance Pointer-Based Gesture Engine (Drag & Resize) ---
    function initElementEvents(el) {
        ...
        el.addEventListener('pointerdown', (e) => {
            ...
            // Bypass gesture tracking if user intends to select text inside an active module on iPad
    ...
    // --- 5. Selection Synchronization Logic ---
    ...
    function setAnimation(animName) {
        ...
        void selectedElement.offsetWidth; // Trigger layout reflow
    ...
    // --- 7. PDF Advanced Ingestion Module ---
    ...
    if (pdfFileInput) {
        ...
        pdfFileInput.onchange = async (e) => {
            ...
            reader.onload = async function() {
                try {
                    if (!window.pdfjsLib) { alert("Core PDF library missing."); return; }
                    ...
                    alert("Failed to parse incoming PDF structure.");
                }
            };
    ...
    // --- 9. Document Exporters (PDF / PPTX Compile Engines) ---
    bindClick('export-pdf-btn', async () => {
        try {
            if (!window.jspdf || !window.html2canvas) { alert("Missing compile libraries."); return; }
            deselect();
            ...
            alert("Error compiling binary PDF output.");
        }
    });

    bindClick('export-pptx-btn', () => {
        try {
            if (!window.PptxGenJS) { alert("PPTX component unavailable."); return; }
            deselect();
            ...
            alert("Error rendering PPTX layout.");
        }
    });

    // --- 10. Stage Fullscreen Mode Router ---
