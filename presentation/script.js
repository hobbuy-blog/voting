...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (e) => {
        if (!colorFloat.contains(e.target) && !e.target.closest('.color-trigger-btn')) colorFloat.classList.add('hidden');
    });
    ...
    bindClick('undo-btn', () => {
        ...
        s.redoStack.push(canvas.innerHTML); canvas.innerHTML=s.undoStack.pop();
        reattachEvents(); updateUndoRedoUI();
    });
    bindClick('redo-btn', () => {
        const s=slides[currentSlideIndex]; if(!s.redoStack.length) return;
        s.undoStack.push(canvas.innerHTML); canvas.innerHTML=s.redoStack.pop();
        reattachEvents(); updateUndoRedoUI();
    });
    ...
    container?.addEventListener('touchstart', e => {
        if (e.touches.length===2) { e.preventDefault(); tDist=Math.hypot(e.touches[0].clientX-e.touches[1].clientX, e.touches[0].clientY-e.touches[1].clientY); tZoom=currentZoom; }
    }, {passive:false});
    container?.addEventListener('touchmove', e => {
        if (e.touches.length===2 && tDist>0) { e.preventDefault(); const d=Math.hypot(e.touches[0].clientX-e.touches[1].clientX, e.touches[0].clientY-e.touches[1].clientY); currentZoom=Math.min(2.0,Math.max(0.1,tZoom*(d/tDist))); applyZoom(); }
    }, {passive:false});
    ...
    function getSnapPositions() {
        const snaps={x:[],y:[]};
        SNAP_PERCENTS.forEach(p=>{ snaps.x.push(Math.round(p*currentWidth)); snaps.y.push(Math.round(p*currentHeight)); });
        return snaps;
    }
    ...
    function initElementEvents(el) {
        ...
        document.addEventListener('pointermove', e=>{
            ...
            if (isRes) {
                ...
                if (dir==='se'){newWidth=Math.max(MIN,sw+dx); if(!isText) newHeight=Math.max(MIN,sh+dy);}
                else if(dir==='sw'){newWidth=Math.max(MIN,sw-dx); if(newWidth>MIN) el.style.left=(ox+dx)+'px'; if(!isText) newHeight=Math.max(MIN,sh+dy);}
                else if(dir==='ne'){newWidth=Math.max(MIN,sw+dx); if(!isText){newHeight=Math.max(MIN,sh-dy); if(newHeight>MIN) el.style.top=(oy+dy)+'px';}}
                else if(dir==='nw'){newWidth=Math.max(MIN,sw-dx); if(newWidth>MIN) el.style.left=(ox+dx)+'px'; if(!isText){newHeight=Math.max(MIN,sh-dy); if(newHeight>MIN) el.style.top=(oy+dy)+'px';}}
                // ★ 水平方向の専用ハンドル処理
                else if(dir==='e') {newWidth=Math.max(MIN,sw+dx);}
                else if(dir==='w') {newWidth=Math.max(MIN,sw-dx); if(newWidth>MIN) el.style.left=(ox+dx)+'px';}
                
                el.style.width=newWidth+'px'; 
                if(!isText) el.style.height=newHeight+'px';
                
                // ★ 斜めリサイズ時にフォントサイズを比例拡大
    ...
    function repositionFloatMenu() {
        ...
        const elL=selectedElement.offsetLeft; const elT=selectedElement.offsetTop;
        const menuH=42; 
        const cRect=canvas.getBoundingClientRect(); const ctRect=container.getBoundingClientRect();
        const cx=(cRect.left - ctRect.left) + elL*currentZoom;
        const cy=(cRect.top  - ctRect.top ) + elT*currentZoom - menuH - 6;
        floatMenu.style.left=Math.max(0,cx)+'px'; floatMenu.style.top =Math.max(0,cy)+'px';
    }
    ...
    function syncPropsPanel() {
        ...
        const tSec=document.getElementById('text-props'); const sSec=document.getElementById('shape-props');
        ...
        const isText=selectedElement.classList.contains('text-element');
        tSec?.classList.toggle('hidden',!isText); sSec?.classList.toggle('hidden',isText);
    ...
    document.getElementById('prop-fontsize')?.addEventListener('change',e=>{ const inner=selectedElement?.querySelector('.content-wrapper'); if(inner) inner.style.fontSize=e.target.value+'px'; });
    document.getElementById('prop-fontfamily')?.addEventListener('change',e=>{ const inner=selectedElement?.querySelector('.content-wrapper'); if(inner) inner.style.fontFamily=e.target.value; });
    document.getElementById('prop-opacity')?.addEventListener('input',e=>{ if(!selectedElement) return; selectedElement.style.opacity=e.target.value/100; document.getElementById('prop-opacity-val').textContent=e.target.value+'%'; });
    ...
    ['bold-btn','float-bold'].forEach(id=>bindClick(id,()=>{ const inner=selectedElement?.querySelector('.content-wrapper'); if(inner) patchText('fontWeight',window.getComputedStyle(inner).fontWeight==='700'?'400':'700'); }));
    ['italic-btn','float-italic'].forEach(id=>bindClick(id,()=>{ const inner=selectedElement?.querySelector('.content-wrapper'); if(inner) patchText('fontStyle',window.getComputedStyle(inner).fontStyle==='italic'?'normal':'italic'); }));
    ...
    // ── Designタブ ───────────────────────────────────────────
    document.querySelectorAll('.theme-swatch').forEach(sw=>{ sw.onclick=()=>{ saveState(); applySlideBackground(sw.dataset.bg, sw.dataset.bg.includes('gradient')); }; });
    function applySlideBackground(bg, isGrad) {
        if(!canvas) return; const s=slides[currentSlideIndex];
        s.bg=bg; s.bgStyle=isGrad?'gradient':'color';
        if(isGrad){canvas.style.background=bg;canvas.style.backgroundImage=bg;}else{canvas.style.background=bg;canvas.style.backgroundImage='none';}
    ...
    document.querySelectorAll('.trans-btn').forEach(btn=>{ btn.onclick=()=>{ slides[currentSlideIndex].transition=btn.dataset.trans; document.querySelectorAll('.trans-btn').forEach(b=>b.classList.remove('active')); btn.classList.add('active'); }; });
    function setAnimation(name) {
        if(!selectedElement) return; saveState();
        selectedElement.setAttribute('data-animation',name);
        const delay=parseFloat(document.getElementById('anim-delay')?.value||0); selectedElement.setAttribute('data-anim-delay',delay); selectedElement.style.animationDelay=delay+'s';
    ...
    document.getElementById('anim-delay')?.addEventListener('change',()=>{ if(!selectedElement) return; const v=parseFloat(document.getElementById('anim-delay').value)||0; selectedElement.setAttribute('data-anim-delay',v); selectedElement.style.animationDelay=v+'s'; });
    ...
    function drawHRuler() {
        ...
        const w=c.offsetWidth; const h=c.offsetHeight; c.width=w; c.height=h;
        const ctx=c.getContext('2d'); ctx.clearRect(0,0,w,h); ctx.fillStyle='#f8fafc'; ctx.fillRect(0,0,w,h);
        ...
        const scale=currentZoom; const step=getTickStep(scale); const offsetLeft=canvas.getBoundingClientRect().left - c.getBoundingClientRect().left;
        for(let v=0;v<=currentWidth;v+=step){ const sx=offsetLeft+v*scale; if(sx<0||sx>w) continue; ctx.beginPath(); ctx.moveTo(sx,h); ctx.lineTo(sx,h/2); ctx.strokeStyle='#cbd5e1'; ctx.lineWidth=1; ctx.stroke(); if(v%(step*5)===0) ctx.fillText(v,sx+2,h-2); }
        SNAP_PERCENTS.forEach(p=>{ const v=Math.round(p*currentWidth); const sx=offsetLeft+v*scale; if(sx<0||sx>w) return; ctx.beginPath(); ctx.moveTo(sx,0); ctx.lineTo(sx,h); ctx.strokeStyle='rgba(37,99,235,0.4)'; ctx.lineWidth=1; ctx.setLineDash([2,2]); ctx.stroke(); ctx.setLineDash([]); });
    }
    function drawVRuler() {
        ...
        ctx.fillStyle='#94a3b8'; ctx.font='9px system-ui';
        const scale=currentZoom; const step=getTickStep(scale); const offsetTop=canvas.getBoundingClientRect().top - c.getBoundingClientRect().top;
        for(let v=0;v<=currentHeight;v+=step){ const sy=offsetTop+v*scale; if(sy<0||sy>h) continue; ctx.save(); ctx.translate(w/2,sy); ctx.rotate(-Math.PI/2); ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(0,-w/2); ctx.strokeStyle='#cbd5e1'; ctx.lineWidth=1; ctx.stroke(); if(v%(step*5)===0) ctx.fillText(v,-18,3); ctx.restore(); }
    }
    function getTickStep(scale) { if(scale>1.5) return 50; if(scale>0.8) return 100; if(scale>0.4) return 200; return 400; }
    ...
    bindClick('word-count-btn',()=>{ const t=getTextContent(),all=t.join(' '); const w=all.trim().split(/\s+/).filter(w=>w).length; const r=document.getElementById('proofread-result'),txt=document.getElementById('proofread-text'); if(txt) txt.textContent=`Slides: ${slides.length}  Words: ${w}  Chars: ${all.replace(/\s/g,'').length}`; if(r) r.style.display='flex'; });
    bindClick('spellcheck-btn',()=>{ const t=getTextContent(); const r=document.getElementById('proofread-result'),txt=document.getElementById('proofread-text'); if(txt) txt.textContent=t.length?`${t.length} text elements / ${t.join('').length} chars`:'No text elements found'; if(r) r.style.display='flex'; });
    ...
    function renderSlideList() {
        ...
        slides.forEach((s,i)=>{
            ...
            const preview=document.createElement('div'); preview.className='thumb-preview-container'; preview.style.width=currentWidth+'px'; preview.style.height=currentHeight+'px';
            preview.innerHTML=(i===currentSlideIndex)?canvas.innerHTML:s.html;
            if(s.bgStyle==='gradient'){preview.style.background=s.bg;} else{preview.style.backgroundColor=s.bg||'#fff';preview.style.backgroundImage='none';}
            const cw=list.clientWidth-4||168; preview.style.transform=`scale(${cw/currentWidth})`;
            const num=document.createElement('div'); num.className='slide-thumb-number'; num.textContent=i+1;
            thumb.appendChild(preview); thumb.appendChild(num); thumb.onclick=()=>switchSlide(i); list.appendChild(thumb);
        });
    }
    ...
    bindClick('delete-slide-btn',()=>{ if(slides.length<=1){alert('最後のスライドは削除できません。');return;} if(!confirm('このスライドを削除しますか？')) return; slides.splice(currentSlideIndex,1); const next=Math.max(0,currentSlideIndex-1); currentSlideIndex=next; const s=slides[next]; canvas.innerHTML=s.html; if(s.bgStyle==='gradient'){canvas.style.background=s.bg;canvas.style.backgroundImage=s.bg;} else{canvas.style.background=s.bg||'#fff';canvas.style.backgroundImage='none';} reattachEvents(); renderSlideList(); updateUndoRedoUI(); });
            ...
            reader.onload=async ev=>{
                try{
                    ...
                    for(let i=1;i<=pdf.numPages;i++){
                        ...
                        // ★ 1ページ目で現在のキャンバスが空なら上書き
                        if (i === 1 && slides.length === 1 && canvas.innerHTML.trim() === '') {
                            slides[0].html = html;
                        } else {
                            imported.push({html,bg:'#ffffff',bgStyle:'color',transition:'none',undoStack:[],redoStack:[]});
                        }
                    }
                    if(imported.length>0) {
                        slides=slides.concat(imported);
                    }
                    switchSlide(0);
                        ...
                        if(spTree){
                            ...
                            shapes.forEach(sp=>{
                                try{
                                    ...
                                    if(!off||!ext) return;
                                    const x=emu2px(parseInt(off.getAttribute('x')||0)), y=emu2px(parseInt(off.getAttribute('y')||0));
                                    const w=emu2px(parseInt(ext.getAttribute('cx')||0)), h=emu2px(parseInt(ext.getAttribute('cy')||0));
                                    const scaleX=currentWidth/1280; const scaleY=currentHeight/720;
                                    const sx=Math.round(x*scaleX), sy=Math.round(y*scaleY), sw=Math.round(w*scaleX), sh=Math.round(h*scaleY);
                                    if(sw<4||sh<4) return;
                                    ...
                                    if(txBody) {
                                        txBody.querySelectorAll('p').forEach(p => {
                                            let pText = '';
                                            p.querySelectorAll('r').forEach(r => { const t = r.querySelector('t'); if (t) pText += t.textContent; });
                                    ...
                                    if(textContent && textContent.trim().replace(/<br>/g,'')){
                                        let fontSize=24; const rPr=txBody?.querySelector('rPr'); if(rPr){const sz=rPr.getAttribute('sz'); if(sz) fontSize=Math.round(parseInt(sz)/100);}
                                        html+=`<div class="canvas-element text-element" style="left:${sx}px;top:${sy}px;width:${sw}px;height:auto;z-index:${maxZIndex++};touch-action:none;" data-animation="none" data-anim-delay="0"><div class="resize-handle nw" data-direction="nw"></div><div class="resize-handle ne" data-direction="ne"></div><div class="resize-handle sw" data-direction="sw"></div><div class="resize-handle se" data-direction="se"></div><div class="resize-handle w" data-direction="w"></div><div class="resize-handle e" data-direction="e"></div><div class="content-wrapper" contenteditable="true" style="font-size:${fontSize}px;color:#0f172a;font-family:'Noto Sans JP',sans-serif;text-align:left;">${textContent}</div></div>`;
                                    } else {
                                        html+=`<div class="canvas-element rect-element" style="left:${sx}px;top:${sy}px;width:${sw}px;height:${sh}px;z-index:${maxZIndex++};touch-action:none;" data-animation="none" data-anim-delay="0"><div class="resize-handle nw" data-direction="nw"></div><div class="resize-handle ne" data-direction="ne"></div><div class="resize-handle sw" data-direction="sw"></div><div class="resize-handle se" data-direction="se"></div><div class="content-wrapper" style="background-color:${fillColor};border-radius:3px;"></div></div>`;
                                    }
                                }catch(_){}
                            });
                        }
                        const bgClrEl=doc.querySelector('bgClr')||doc.querySelector('*|bgClr');
                        let bg='#ffffff'; if(bgClrEl){const srgb=bgClrEl.querySelector('srgbClr'); if(srgb) bg='#'+(srgb.getAttribute('val')||'ffffff');}
                        
                        if(i===0 && slides.length===1 && canvas.innerHTML.trim()==='') {
                            slides[0].html = html; slides[0].bg = bg;
                        } else {
                            slides.push({html,bg,bgStyle:'color',transition:'none',undoStack:[],redoStack:[]});
                        }
                    }
                    switchSlide(0);
    ...
    bindClick('export-pdf-btn', async ()=>{
        ...
        slides[origIdx].html=canvas.innerHTML;

        // ★ ズーム・transform を完全に解除してキャプチャズレ(グレー帯)を防止
    ...
    bindClick('export-pptx-btn',()=>{
        ...
        slides.forEach(s=>{
            ...
            // ★ 背景色を確実にセット（透明・未設定は白）
            const bgHex = (s.bg && s.bg !== 'transparent' && !s.bg.replace(/\s/g, '').includes('rgba(0,0,0,0)')) ? rgbToHex(s.bg) : '#ffffff';
            sl.background={fill:bgHex.replace('#','')};
    ...
    function presGoNext() { if(currentSlideIndex<slides.length-1){ currentSlideIndex++; buildPresenterSlide(currentSlideIndex); } }
    function presGoPrev() { if(currentSlideIndex>0){ currentSlideIndex--; buildPresenterSlide(currentSlideIndex); } }
    ...
    presOverlay.addEventListener('touchstart', e=>{ if(e.touches.length===1){ swipeStartX=e.touches[0].clientX; swipeStartY=e.touches[0].clientY; } }, {passive:true});
    presOverlay.addEventListener('touchend', e=>{
        if(e.changedTouches.length===1){
            const dx=e.changedTouches[0].clientX-swipeStartX; const dy=Math.abs(e.changedTouches[0].clientY-swipeStartY);
